    ...
    return (
        <div className="mb-2 pb-4 border-b border-dashed border-gray-300">
            <div className="mb-2 font-semibold text-xs">
                Log
                <span className={`ml-2 px-1 font-normal level-${props.metadata.level}`}>{props.metadata.level}</span>
            </div>
            <code className="code-block mb-4" dangerouslySetInnerHTML={{ __html: props.label }} />
            <DefinitionList title="">
                {Object.entries(props.context).map(([key, value]) => (
            ...
            {query.split(placeholder).reduce((prev, current, i) => {
                ...
                return prev.concat(
                    <span className="mt-2 inline-flex group" key={i}>
                        <CopyableCode>
                            <span className="text-purple-500">{stringifyOccurrenceData(replacement.value)}</span>
                        </CopyableCode>
                    ...
                    {Object.entries(props.context).map(([key, value]) => (
                        <React.Fragment key={key}>
                            ...
                            <dd className="flex items-baseline">
                                ...
                                {value.type !== 'int' && value.type !== 'float' && (
                                    <CopyableCode>
                                        <span className="text-purple-500">{stringifyOccurrenceData(value.value)}</span>
                                    </CopyableCode>
                                )}
                                <span className="ml-2 mr-auto text-gray-700 text-xxs uppercase">{value.type}</span>
                            </dd>
                        </React.Fragment>
                    ))}
                </DefinitionList>
