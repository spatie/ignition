            ...
            <div className={`code-inline ${codeClassName} transition-all pr-8`} style={{ color: 'inherit' }}>
                ...
                <button
                    className={`absolute top-0 right-0 mr-2 ${
                        copied ? 'opacity-100' : 'opacity-75 group-hover:opacity-100 group-hover:transition-opacity'
                    }`}
                    onClick={copy}
                    title="Copy to clipboard"
                >
                    <i className={`${copied ? 'fa fa-clipboard-check text-green-500' : 'far fa-clipboard'}`} />
                </button>
                {copied && (
                    <p
