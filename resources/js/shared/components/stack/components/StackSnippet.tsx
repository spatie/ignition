...
export default function StackSnippet({ code, highlightedLineNumber, selectedRange, onSelectRange }: Props) {
    ...
    function handleLineNumberClick(e: React.MouseEvent, lineNumber: number) {
        if (e.shiftKey && firstSelectedLineNumber !== null) {
            onSelectRange([firstSelectedLineNumber, lineNumber].sort((a, b) => a - b) as [number, number]);
        } else {
                ...
                <div className="stack-lines">
                    {Object.keys(code).length &&
                        Object.keys(code).map((lineNumber) => {
                            return (
                                <p
                                    onClick={(event) => handleLineNumberClick(event, parseInt(lineNumber))}
                                    key={lineNumber}
                                    className={`cursor-pointer stack-line ${
                                        withinSelectedRange(parseInt(lineNumber)) ? 'stack-line-selected' : ''
                                    } ${parseInt(lineNumber) === highlightedLineNumber ? 'stack-line-highlight' : ''}`}
                                >
                                    {lineNumber}
                                </p>
                            );
